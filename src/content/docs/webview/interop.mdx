...
Therefore it can be difficult to pass in code that is not known at compile-time _(as strings will be serialized for JavaScript and will thus contain quotes)_.
...
```cpp title="Example: Throwing an exception"
...
:::note
This will (obviously) not work when compiling with `-fno-exceptions` and one can also disable this behavior explicitly (see [CMake Configuration](../getting-started/cmake))
:::
